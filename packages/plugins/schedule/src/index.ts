...
schedulePlugin.command('定时列表')
  .action(({adapter,bot,message}) => {
    const schedules = schedulePlugin.jsondb.filter<Schedule>('schedule',(schedule)=>{
      return schedule.adapter === adapter.name &&
...
schedulePlugin.command('添加定时')
  .action(async ({adapter,prompt,bot,message})=>{
...
schedulePlugin.command('删除定时 <no:number>')
