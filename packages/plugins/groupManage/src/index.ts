...
groupManage.command('setAdmin [user_id:user_id]')
  .desc('设置/取消设置群管理')
...
groupManage.command('setTitle <user_id:user_id> <title:string>')
  .desc('设置/取消设置群头衔')
  .permission('admin')
...
groupManage.command('setNotice <notice:string>')
  .desc('设置设置群公告')
  .permission('admin')
  .scope('group')
...
groupManage.command('setAnonymous')
  .desc('开启/关闭群匿名')
  .permission('admin')
  .scope('group')
  .option('-c <cancel:boolean>', false)
...
groupManage.command('setCard [user_id:user_id] [card:string]')
.desc('设置/取消设置群名片')
.permission('admin')
  .scope('group')
...
groupManage.command('addMe [comment:string]')
  .desc('机器人主动加你为好友')
  .scope('group')
...
groupManage.command('setName [name:string]')
  .desc('修改群名称')
  .permission('admin')
  .scope('group')
...
groupManage.command('sendPoke [user_id:user_id]')
  .desc('发送戳一戳')
  .alias('戳')
  .scope('group')
  .action<ICQQAdapter>(async ({ bot, message }, user_id) => {
