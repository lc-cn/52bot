import { Message, Plugin } from '52bot';
import type {} from '@52bot/plugin-jsondb';
...
qaPlugin.command('问答 <question:string> <answer:string>')
...
qaPlugin.command('删除问答 <no:number>')
...
qaPlugin.command('问答列表')
  ...
  .action(({adapter,bot,prompt,message,options}) => {
    ...
    return `共收录了${qaList.length}个问答\n`+pageAfter.map((qa, index) => {
      return `${index+1}：\n问:${qa.content}\n答:${qa.answer}\n`;
