...
export class Ws {
  ...
  // 创建一个websocket连接
  createWebsocket(wsData: WsAddressObj,access_token:string) {
    // 先链接到ws
    this.connectWs(wsData,access_token);
    // 对消息进行监听
    return this.createListening(access_token);
  }

  // 创建监听
  createListening(access_token:string) {
    // websocket连接已开启
    this.ws.on('open', () => {
    ...
    this.ws.on('message', (data: wsResData) => {
      ...
      if (wsRes?.op === OpCode.HELLO && wsRes?.d?.heartbeat_interval) {
        ...
        // 非断线重连时，需要鉴权
        this.isReconnect ? this.reconnectWs(access_token) : this.authWs(access_token);
        return;
      }
  ...
  // 连接ws
  connectWs(wsData: WsAddressObj,access_token) {
    // 创建websocket连接
  ...
  // 鉴权
  authWs(access_token:string) {

    // 鉴权参数
    const authOp = {
      op: OpCode.IDENTIFY, // 鉴权参数
      d: {
        token: `QQBot ${access_token}`, // 根据配置转换token
  ...
  // 重新重连Ws
  reconnectWs(access_token:string) {
