...
guildManager.command('/置顶 [message_id:string]')
    ...
    .action<QQAdapter>(async ({message}, message_id) => {
        ...
        const result = await message.bot.pinChannelMessage(message.channel_id, message_id)
...
guildManager.command('/取消置顶 [message_id:string]')
    ...
    .action<QQAdapter>(async ({message}, message_id) => {
        ...
        const result = await message.bot.unPinChannelMessage(message.channel_id, message_id)
...
guildManager.command('/设为公告 [message_id:string]')
    ...
    .action<QQAdapter>(async ({message}, message_id) => {
        ...
        const result = await message.bot.setChannelAnnounce(message.guild_id, message.channel_id, message_id)
...
guildManager.command('/禁言 [user_id:user_id]')
    ...
    .action<QQAdapter>(async ({message, options}, user_id) => {
        const result = await message.bot.muteGuildMember(message.guild_id, user_id, options.time)
